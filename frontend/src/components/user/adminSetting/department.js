...
function Row(props) {
  ...
  const update= (iddepartment)=>{
    //Refer registration.js in routes folder in backend 
    Axios.put("http://161.97.79.224:3001/register/updateDepartment",{
      name: name,
   ...
   //Refer registration.js in routes folder in backend 
  //Delete department deatils used in delete Icon Button
  const Delete = (iddepartment)=>{
    if (window.confirm("Are you sure you want to delete?")) {
    Axios.put("http://161.97.79.224:3001/register/deleteDepartment",{
      iddepartment:iddepartment,
    }).then((response)=> {
        console.log(response.data)
     alert('Deleted');
  ...
  return (
    <React.Fragment>
     <StyledTableRow>
        ...
        <StyledTableCell align="center">
            <IconButton onClick={()=>{Delete(row.iddepartment)}} > <DeleteIcon/> </IconButton>
        </StyledTableCell>
        <StyledTableCell align="center">
         {editable?<IconButton onClick={()=>{update(row.iddepartment)}}> <CheckIcon/> </IconButton>
         :
...
export default function CollapsibleTable() {
    ...
    const handleClose = () => {
       ...
       //new department adding
       const handleSubmit = (e) =>{
        e.preventDefault();
        Axios.post("http://161.97.79.224:3001/register/department",{
          name: name,
        }).then((response)=> {
    ...
    <div>
         <Dialog open={dopen} TransitionComponent={Transition} onClose={handleCloseDialog} aria-labelledby="responsive-dialog-title">
          ...
          <form autoComplete="off" onSubmit={handleSubmit}>
            <TextField id="Name" required value={name} label="Name"  style={{ margin: 8 }} placeholder="None" helperText="Please enter department name" fullWidth  margin="normal" InputLabelProps={{ shrink: true, }} variant="outlined" onChange={(e)=>{ setName(e.target.value)}} />
