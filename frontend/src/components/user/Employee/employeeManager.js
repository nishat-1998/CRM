...
import {Grid,Button,MenuItem,IconButton,TextField,Table,TableBody,TableCell,TableContainer,TableHead,TableRow,Paper,Typography,Link} from '@material-ui/core';
...
export default function CollapsibleTable() {
  ...
  const {id} = useParams();
    ...
    const headers = [
      ...
      { label: "Total customer", key: "total_customer" }
    ];
  ...
  const handleChange = () => {
    console.log(id)
    Axios.get("http://161.97.79.224:3001/employee/getemployeeforManager",{
      params:{
        manager_id:id
      }
    }).then((response)=> {
